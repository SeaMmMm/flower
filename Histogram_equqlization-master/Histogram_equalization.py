...
class HistogramEqualization(object):
    def __init__(self, img_file):
        ...
        self.Histogram = np.zeros((1, 256), dtype=int)
        self.NewHistogram = np.zeros((1, 256), dtype=int)
    ...
    def draw_cd_f(self):
        ...
        for i in range(256):
            if i == 0:
                ...
                self.Cumulative_distribution[0][i] = (
                    self.Histogram[0][i] / N + self.Cumulative_distribution[0][i - 1]
                )
        # 绘制图像
        x = list(range(256))
        plt.figure()
        ...
        plt.title("累积分布函数图像")
        plt.plot(
            x, self.Cumulative_distribution[0, :] * 255, color="green", linewidth=0.5
        )
    ...
    def equalization(self, filename="./result/new_img.jpg"):
        ...
        f = np.zeros((1, 256), dtype=int)  # 映射函数
        alpha = 1  # 混合参数
        for i in range(256):
